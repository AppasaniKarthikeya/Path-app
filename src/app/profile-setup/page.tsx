            ...
            <div style={styles.field}>
              ...
              <div style={styles.optionGrid}>
                {[
                  ...
                  >
                    <span style={{...styles.optionLabel, color: profile.status === opt.value ? '#fff' : 'var(--brand-dark-purple)'}}>{opt.label}</span>
                    <span style={{...styles.optionDesc, color: profile.status === opt.value ? 'rgba(255,255,255,0.8)' : 'var(--brand-dark-purple)'}}>{opt.desc}</span>
                  </button>
                ))}
              </div>
            </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  optionLabel: {
    fontSize: 15,
    fontWeight: 700,
    color: 'inherit', // Will inherit white when selected, dark purple when not. Wait, buttons don't inherit well. We will handle color inline or just use inherit.
  },
  optionDesc: {
    fontSize: 12,
    opacity: 0.8,
